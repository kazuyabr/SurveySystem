...
import com.surveysystem.entity.CPFValidationStatus;
import com.surveysystem.entity.Poll;
import com.surveysystem.entity.PollStatus;
import com.surveysystem.entity.PollVote;
import com.surveysystem.entity.Response;
import com.surveysystem.repository.AssociadoRepository;
import com.surveysystem.repository.PollRepository;
import com.surveysystem.service.PollVoteService;
...
public class PollVoteController {
	...
	/**
	 * Persiste uma votacao no sistema - Apenas 1 votacao por cpf e permitida - So
	 * pode votar em enquetes abertas - Apos votar, se o mesmo cpf tentar votar
	 * novamente, vai apenas atualizar seu voto
	 * 
	 * @param pollVote
	 * @param result
	 * @return
	...
	private ResponseEntity<Response<PollVoteDto>> store(@RequestBody @Valid PollVote pollVote, BindingResult result) {
		...
		if (result.hasErrors()) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Response<>(null, result.getAllErrors()
		...
		try {
			...
			if (associado.isPresent() && pollVoteService.checkCPF(associado.get().getCpf())
					.getStatus() == CPFValidationStatus.ABLE_TO_VOTE) {
				return ResponseEntity.status(HttpStatus.CREATED)
						.body(new Response<>(pollVoteService.store(pollVote), null));
			}
